...
function statusFromText(text: string): SourceResult["status"] {
  ...
  if (/erişime engellenmiştir|erişim engeli|engellenmiştir|aile\s+ve\s+çocuk\s+profilinde\s+görüntülenememektedir|aile\s+(erişim|profil|güvenli internet)|aile\s+engeli|family\s+(access|filter|profile)|has been blocked|uygulanmakta olan kararlar|erişilmemektedir|erişilmesin|zararlı içerik/i.test(text)) return "blocked";
